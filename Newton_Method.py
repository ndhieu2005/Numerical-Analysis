...
def newton_method(f_str, a, b, n):
    # Khởi tạo biến x và biểu thức toán học
    x = sp.symbols('x')
    
    # Chuyển các chuỗi hàm thành biểu thức toán học
    f_expr = sp.sympify(f_str)
    df_expr = sp.diff(f_expr, x)  # Đạo hàm bậc 1
    d2f_expr = sp.diff(df_expr, x)  # Đạo hàm bậc 2
    ...
    for _ in range(n):
        ...
        if abs(x_new - x0) < 1e-6:
            print(f"Sai số nhỏ hơn ngưỡng, dừng lại.")
            return x_new
        
        # Cập nhật x0 cho vòng lặp tiếp theo
        x0 = x_new

    print(f"Không hội tụ sau {n} vòng lặp.")
    return None

# Nhập các chuỗi hàm f(x) từ người dùng
...
result = newton_method(f_str, a, b, n)
